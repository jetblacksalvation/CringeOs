...
#include <limine.h>
#endif
#define MAX_ASCII_LEN 16
#define revY uint64_t y = MAX_ASCII_LEN; y >0; y--
#define nrevY uint64_t y = 0; y <MAX_ASCII_LEN; y++
class Console{
public:
        ...
        for(nrevY){
            for(uint8_t x =8; x>0; x--){
                ...
                if(isbit)
                {
                    ...
                    fb_ptr[(y+_y) * (framebuffer->pitch / 4) + (_x-x)] = 0xffffff;
